	...
	K: Copy + Eq + Hash,
{
	...
	fn eviction(&mut self, max_size: CacheSize) -> Option<K> {
		match self {
			PolicyStackType::Lfu(stack) => stack.eviction(max_size),
			PolicyStackType::Fifo(stack) => stack.eviction(max_size),
			PolicyStackType::Lru(stack) => stack.eviction(max_size),
			PolicyStackType::Mru(stack) => stack.eviction(max_size),
		}
	}
}
